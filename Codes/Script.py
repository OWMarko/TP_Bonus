...
#print("Prédictions :", y_pred_tree)
#print("Étiquettes :", y_test)

#La fonction perte F
tree_loss = np.mean(np.abs(y_pred_tree - y_test))
print("Fonction perte de l'arbre sur base test :", tree_loss)
...
"""
Nous remarquons que la perte moyenne de l'arbre de décision est minimale, 0.0200, comparée à la perte moyenne de notre réseau de neurones simple.
Cette différence est due à la simplicité de notre réseau, qui ne prend pas suffisamment en compte les différentes subtilités dans les données,
tandis que l'arbre de décision s'adapte aux diverses subtilités.
"""
...
https://github.com/OWMarko/TP_Bonus/blob/main/Codes/Question5.py

Cependant, nous remarquons que pour un grand nombre de couches cachées, nous perdons la convergence de notre perte. 
Nous pouvons conclure que pour un grand nombre de couches cachées, le réseau de neurones est inefficace.
"""
